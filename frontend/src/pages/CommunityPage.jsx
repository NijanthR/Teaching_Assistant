...
function CommunityPage() {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const response = await fetch(apiUrl(`/api/community/messages/?name=${encodeURIComponent(currentName)}`))
  ...
  const handleSend = () => {
    ...
    fetch(apiUrl('/api/community/messages/post/'), {
      method: 'POST',
  ...
  const handleDelete = (messageId) => {
    fetch(apiUrl('/api/community/messages/delete/'), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        id: messageId,
